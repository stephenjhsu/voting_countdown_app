...
class Window(object):
    ...
    def __init__(self, message='', title='', default_text='', ok=None, cancel=None, dimensions=(0, 0),
                 secure=False, icon=None, icon_path=None):
        #icon="data/alarm_clock-128.png"):
        message = text_type(message)
        message = message.replace('%', '%%')
        title = text_type(title)
        ...
        #icon_path = "data/alarm_clock-128.png"
        if icon_path is not None:
            try:
                icon = _nsimage_from_file(icon_path)
                self._alert.setIcon_(icon)
...
class App(object):
    ...
    def __init__(self, name, title=None, icon=None, template=None, menu=None, quit_button='Quit'):
        _require_string(name)
        self._name = name
        self._icon = self._icon_nsimage = self._title = icon
        #print(icon)
        self._template = template
        self.icon = icon
        self.title = title
